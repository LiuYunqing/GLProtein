...
def generate_PROBE_embedding(file_path: str,
    ...
    # output_csv = pd.concat([output_csv['Entry'], output_csv['Embedding'].apply(pd.Series)], axis = 1).rename(columns = {i:str(i) for i in range(len(protein_embeddings[0]))}) 
